...
while(i<1458){
########################################
dnn <- sae.dnn.train(train.x, train.y, hidden = sol$values[j],learningrate=sol$values[i],batchsize=sol$values[k])
## predict by dnn
...
sonuc <- performance.calculation(test.y, predicted, threshold=0.5)
i <- i+3
j <- j+3
k <- k+3
...
sonucListGenel
yol <- "D:/makaleler/deepLearnTuning/dokumanlar/stacked/gridResults/"
yol <- paste(yol,"grid",dosya,sep="")
